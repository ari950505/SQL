...
--8. 2003년도 입사한 사원의 이름과 입사일 그리고 급여를 출력하세요. (BETWEEN)
SELECT FIRST_NAME,LAST_NAME , HIRE_DATE, SALARY
FROM EMPLOYEES
WHERE HIRE_DATE BETWEEN '03/01/01' AND '03/12/31';

--9. 2003년도 입사한 사원의 이름과 입사일 그리고 급여를 출력하시오. (LIKE)
SELECT FIRST_NAME,LAST_NAME ,HIRE_DATE, SALARY
FROM EMPLOYEES
WHERE HIRE_DATE LIKE '03/%/%';

--10.모든 사원의 이름과 급여를 급여가 많은 사원부터 적은 사원순서로 
SELECT FIRST_NAME, LAST_NAME , SALARY 
FROM EMPLOYEES
ORDER BY SALARY DESC;

--11. 위 질의를 60번 부서의 사원에 대해서만 질의하세요ㄴ
SELECT FIRST_NAME, LAST_NAME , SALARY 
FROM EMPLOYEES
WHERE DEPARTMENT_ID = 60
ORDER BY SALARY DESC;

--12. 직무아아디가 IT_PROG이거나  SA_MAN 인 사원의 이름과 직무아이디를 출력
SELECT FIRST_NAME, LAST_NAME , JOB_ID
FROM EMPLOYEES
WHERE JOB_ID = 'IT_PROG' OR JOB_ID = 'SA_MAN';
--WHERE JOB_ID IN ('IT_PROG', 'SA_MAN');

--13. Steven King 사원의 정보를 'Steven King 사원의 급여는 24000달러입니다' 형식으로 출력.
SELECT FIRST_NAME||' '|| LAST_NAME ||' '||'사원의 급여는'||' '|| SALARY || '달러입니다' AS SENTENCE
FROM EMPLOYEES
WHERE FIRST_NAME = 'Steven' AND LAST_NAME = 'King';

--14. 매니저(MAN)직무에 해당하는 사원의 이름과 직무아이디를 출력
SELECT  FIRST_NAME,LAST_NAME, JOB_ID 
FROM EMPLOYEES
WHERE JOB_ID LIKE '%MAN%';

--15. 매니저 직무에 해당하는 사원의 이름고 직무아이디를 직무아이디 순서대로 출력.
SELECT  FIRST_NAME,LAST_NAME , JOB_ID 
FROM EMPLOYEES
WHERE JOB_ID LIKE '%MAN%'  
ORDER BY JOB_ID ASC;
